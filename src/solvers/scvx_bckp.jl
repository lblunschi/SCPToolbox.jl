...
function solve(
    ...
    while true
        # last_λ = pbm.pars.λ
        # weight = min(round(k/10),6)
        # pbm.pars.λ = 10^(weight)
        # println(pbm.pars.λ)
        # ref.J_pen = ref.J_pen*pbm.pars.lambda
        # print(ref.J_aug)
        # print(ref.J_pen)
        ...
        try
            ...
            # sol_it = spbm.sol
            # veh = pbm.traj.mdl.vehicle
            # traj = pbm.traj.mdl.traj
            # tdil = sol_it.p[veh.id_t]

            # @show tdil
            # @show tdil - traj.tf_max
            # @show traj.tf_min - tdil
            println("GOT out")
...
function actual_cost_penalty!(sol::SubproblemSolution, pbm::SCPProblem)::RealTypes
    ...
    Λ = [1, 1, 1, 100, 1, 1, 1, 200]*0.1
    W = LinearAlgebra.diagm(Λ) / N

    # Λf = [10, 10, 10, 10, 1, 1, 1, 1]
    # Wf = LinearAlgebra.diagm(Λf) / N
...
function print_info(spbm::Subproblem, err::Union{Nothing,SCPError} = nothing; xc=missing, td=missing)::Nothing
    ...
    elseif unsafe_solution(sol)
        ...
        # defect =  scale.iSx * spbm.sol.defect
        # defect_max = norm(defect, Inf)
        println("MIN vd", minimum(sol.vd))
        println("MIN vd", maximum(sol.vd))
        rollout_err_max = 999
        if !ismissing(xc)
            ...
            rollout_err_inf = norm(rollout_errs, Inf)
            println("Rollout error maximum: ", rollout_err_inf)
        ...
        names = ["x","y","z","yaw","u","v","w","r"]

        """println("Virtual control norms and max abs values:")
        for i in 1:8
            println(
                names[i],
                ":  ||vd||₂ = ", norm(sol.vd[i,:], 2),
                "   max|vd| = ", maximum(abs.(sol.vd[i,:]))
            )
        end"""

        #println("Slack ibc: ", sol.vic, " fbc: ", sol.vtc)
