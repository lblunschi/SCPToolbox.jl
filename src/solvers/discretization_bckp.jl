...
function propagate(
    ...
    if method == FOH
        ...
        # errs = zeros(size(xd))
        # for k in 1:size(xd, 2)
        #     errs[:,k] = (sample(xc, td[k]) .- xd[:,k])
        # end
        # @show maximum(errs) argmax(errs)
        # Isx = pbm.common.scale.iSx
        # errs_scaled = similar(errs)
        # for k in 1:size(xd, 2)
        #     errs_scaled[:, k] = Isx * errs[:, k]
        # end
        # @show norm(errs_scaled, 2) maximum(abs.(errs_scaled)) argmax(abs.(errs_scaled))
    elseif method == IMPULSE
