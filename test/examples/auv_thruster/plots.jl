...
function plot_trajectory_history(mdl::AUVProblem,
                                 history::SCPHistory; 
                                 plot_name::Union{Nothing, String} = nothing)::Nothing
    ...
    name = plot_name === nothing ? "AUV_traj_iters" : "$(plot_name)_traj_iters"

    save_figure(name, algo)
...
function plot_final_trajectory_x_y(mdl::AUVProblem,
    ...
    acc = zeros(4, N)
    allocation_matrix = mdl.vehicle.thruster_allocation_matrix
    for k = 1:N
        acc[:, k] = allocation_matrix*thrust[:, k]
    end    
    pos = sol.xd[mdl.vehicle.id_r, :]
    for k = 1:N
        base = pos[1:2, k]
    ...
    name = plot_name === nothing ? "AUV_final_traj_xy" : "$(plot_name)_final_traj_xy"
...
function plot_final_trajectory_x_z(mdl::AUVProblem,
    ...
    name = plot_name === nothing ? "AUV_final_traj_xz" : "$(plot_name)_final_traj_xz"
...
function plot_final_trajectory_y_z(mdl::AUVProblem,
    ...
    name = plot_name === nothing ? "AUV_final_traj_yz" : "$(plot_name)_final_traj_yz"
...
function plot_inputs_ux_uy_uz_uyaw(mdl::AUVProblem,
    ...
    axs = [fig.add_subplot(6, 1, i) for i in 1:6]

    labels = ["\$t_1\$ [N]", "\$t_2\$ [N]", "\$t_3\$ [N]", "\$t_4\$ [N]", "\$t_5\$ [N]", "\$t_6\$ [N]"]
    ...
    ct_u = hcat([sample(sol.uc, τ)[1:6] for τ in ct_τ]...)
    ...
    name = plot_name === nothing ? "AUV_inputs_6ch" : "$(plot_name)_inputs_6ch"
...
function plot_position_xyzyaw_vs_time(mdl::AUVProblem,
    ...
    name = plot_name === nothing ? "AUV_position_4ch" : "$(plot_name)_position_4ch"
...
function plot_velocity_xyzyaw_vs_time(mdl::AUVProblem,
    ...
    name = plot_name === nothing ? "AUV_velocity_4ch" : "$(plot_name)_velocity_4ch"
...
function plot_dynamics_inputs_ux_uy_uz_uyaw_energy(mdl::AUVProblem,
    ...
    name = plot_name === nothing ? "AUV_inputs_4ch" : "$(plot_name)_inputs_4ch"
