...
def load_xd_states(csv_path: str | Path) -> tuple[np.ndarray, np.ndarray]:
    """
    Expected columns:
      t, x1, ..., x8
    """
...
def compute_openloop_rollout(
    uc_csv_path: str | Path,
    xd_csv_path: str | Path,
    tdil: float,
    out_openloop_csv: str | Path = "data_x_openloop.csv",
    out_error_csv: str | Path = "data_openloop_error.csv",
):
    """
    Open-loop rollout:
      - initialize at xd[0]
      - for each interval k -> k+1:
            x_int[k+1] = integrate(x_int[k], t[k], t[k+1])
      - do NOT reset to xd[k] after the first point

    Compare the open-loop rollout against xd on the same time grid.
    """
    hydro = HydroParams()
    ...
    # reference discrete trajectory
    t_xd, x_xd = load_xd_states(xd_csv_path)

    n = len(t_xd)
    if n < 2:
        raise ValueError("Need at least 2 time points")

    x_open = np.zeros_like(x_xd)
    x_open[0, :] = x_xd[0, :]  # only initial condition comes from xd
    ...
    for k in range(n - 1):
        ...
        x_open[k + 1, :] = integrate_one_step(
            t0=t0,
            t1=t1,
            x0=x_open[k, :],   # <- open-loop propagation
            u_of_t=u_of_t,
            hydro=hydro,
            veh=veh,
            tdil=tdil,
        ...
        err = x_open[k + 1, :] - x_xd[k + 1, :]
        error_rows.append({
            "k": k + 1,
            "t": t1,
            **{f"error_{i}": err[i - 1] for i in range(1, 9)},
            "error_norm_2": float(np.linalg.norm(err, ord=2)),
            "error_norm_inf": float(np.linalg.norm(err, ord=np.inf)),
        })

    df_open = pd.DataFrame({
        "t": t_xd,
        **{f"x{i}": x_open[:, i - 1] for i in range(1, 9)},
    })
    df_err = pd.DataFrame(error_rows)

    df_open.to_csv(out_openloop_csv, index=False)
    df_err.to_csv(out_error_csv, index=False)

    return t_xd, x_xd, x_open, df_open, df_err


def plot_openloop_results(
    t_xd: np.ndarray,
    x_xd: np.ndarray,
    x_open: np.ndarray,
    out_xy_path: str | Path = "openloop_xy.png",
    out_states_path: str | Path = "openloop_states.png",
):
    # XY plot
    plt.figure(figsize=(7, 6))
    plt.plot(x_xd[:, 0], x_xd[:, 1], "ko", markersize=5, label="xd")

    plt.plot(x_open[:, 0], x_open[:, 1], "b-", label="open-loop integrated")
    plt.plot(x_open[:, 0], x_open[:, 1], "bx", markersize=4)

    plt.xlabel("x")
    plt.ylabel("y")
    plt.title("XY trajectory: xd vs open-loop integration")
    plt.legend()
    plt.grid(True)
    plt.axis("equal")
    plt.tight_layout()
    ...
    for i in range(8):
        ...
        ax.plot(t_xd, x_open[:, i], "b-", label="open-loop integrated")
        ax.plot(t_xd, x_open[:, i], "bx", markersize=3)
    ...
    axes[-2].set_xlabel("t")
    axes[-1].set_xlabel("t")
    fig.suptitle("Open-loop rollout vs xd", y=0.995)
    fig.tight_layout()
    fig.savefig(out_states_path, dpi=200)
    plt.show()


def plot_openloop_errors(
    df_err: pd.DataFrame,
    out_errors_path: str | Path = "openloop_errors.png",
):
    """
    Plot error_i versus time for all 8 states, plus norm plots.
    """
    t_err = df_err["t"].to_numpy(dtype=float)
    err = df_err[[f"error_{i}" for i in range(1, 9)]].to_numpy(dtype=float)
    ...
    for i in range(8):
        ax = axes[i]
        ax.plot(t_err, err[:, i], "r-", label=f"error in {state_labels[i]}")
        ax.plot(t_err, err[:, i], "ro", markersize=3)
    ...
    axes[8].plot(t_err, df_err["error_norm_2"].to_numpy(dtype=float), "m-")
    axes[8].plot(t_err, df_err["error_norm_2"].to_numpy(dtype=float), "mo", markersize=3)
    axes[8].set_ylabel("||err||_2")
    axes[8].grid(True)

    axes[9].plot(t_err, df_err["error_norm_inf"].to_numpy(dtype=float), "g-")
    axes[9].plot(t_err, df_err["error_norm_inf"].to_numpy(dtype=float), "go", markersize=3)
    axes[9].set_ylabel("||err||_inf")
    axes[9].grid(True)

    axes[-2].set_xlabel("t")
    axes[-1].set_xlabel("t")
    fig.suptitle("Open-loop errors", y=0.995)
    fig.tight_layout()
    fig.savefig(out_errors_path, dpi=200)
    plt.show()
...
if __name__ == "__main__":
    ...
    t_xd, x_xd, x_open, df_open, df_err = compute_openloop_rollout(
        uc_csv_path=base / "data_uc.csv",
        xd_csv_path=base / "data_xd.csv",
        tdil=tdil,
        out_openloop_csv=base / "data_x_openloop.csv",
        out_error_csv=base / "data_openloop_error.csv",
    )

    plot_openloop_results(
        t_xd=t_xd,
        x_xd=x_xd,
        x_open=x_open,
        out_xy_path=base / "openloop_xy.png",
        out_states_path=base / "openloop_states.png",
    )

    plot_openloop_errors(
        df_err=df_err,
        out_errors_path=base / "openloop_errors.png",
    )

    print("Saved:")
    print(base / "data_x_openloop.csv")
    print(base / "data_openloop_error.csv")
    print(base / "openloop_xy.png")
    print(base / "openloop_states.png")
    print(base / "openloop_errors.png")