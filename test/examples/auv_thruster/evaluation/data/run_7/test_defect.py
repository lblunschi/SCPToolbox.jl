...
def compute_stepwise_defects(
    ...
    For each interval k -> k+1:
      - use saved state from xc_csv[k] as initial condition
      - integrate to t[k+1]
      - compare against xc_ref[k+1]
      - defect[k] = x_int[k+1] - x_ref[k+1]

    Assumes xc_csv and xc_ref use the same time grid.
    """
    hydro = HydroParams()
    ...
    # saved trajectory used as step initial conditions
    t_csv, x_csv = load_xc_states(xc_csv_path)

    # reference trajectory used to compute defects
    t_ref, x_ref = load_xc_states(xc_ref_path)

    if len(t_csv) != len(t_ref):
        raise ValueError("data_xc.csv and data_xc.csv do not have the same number of rows")

    if not np.allclose(t_csv, t_ref, atol=1e-12, rtol=0.0):
        raise ValueError("Time grids in data_xc.csv and data_xc.csv do not match")
    ...
    integrated_rows.append({
            "k": 1,
            "t": 0,
            **{f"x1_int_{i}": x_csv[0, :][i-1] for i in range(1, 9)},
        })
    defect_rows = []
    ...
    for k in range(n - 1):
        ...
        x0_saved = x_csv[k, :]        # initial condition from data_xc.csv at step k
        x1_ref = x_ref[k + 1, :]      # reference state from data_xc.csv at step k+1
        ...
        integrated_rows.append({
            "k": k + 2,
            "t": t1,
            **{f"x1_int_{i}": x1_int[i - 1] for i in range(1, 9)},
        })
    ...
    return df_integrated, df_defect
...
if __name__ == "__main__":
    ...
    df_integrated, df_defect = compute_stepwise_defects(
        uc_csv_path=base / "data_uc.csv",
        xc_csv_path=base / "data_xd.csv",              # saved states used as x0 at each step
        xc_ref_path=base / "data_xd.csv",              # reference states for defect computation
        tdil=tdil,
