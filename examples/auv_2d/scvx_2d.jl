...
N = 200 # Number of trajectory segments (N+1 knot points)
Nsub = 100 # Number of segments in each convex subproblem (Nsub+1 knot points)
iter_max = 100 # Maximum number of SCvx iterations
λ = 10^4 # Penalty weight for virtual control and trust region violation
ρ_0 = 0.0 # Initial trust region radius
ρ_1 = 0.1 # Trust region radius contraction threshold
ρ_2 = 0.6 # Trust region radius expansion threshold
β_sh = 1.5 # Trust region radius contraction factor
β_gr = 1.5 # Trust region radius expansion factor
η_init = 1.0 # Initial virtual control and trust region violation tolerance
...
cost_thrust = sum(sol_safe.ud[i]^2 for i in 1:4)
cost_sway = sum(sol_safe.xd[6,t]^2 for t in 1:size(sol_safe.xd, 2))
@printf("Cost breakdown for last safe solution:\n")
@printf("  Thrust cost = %.3e\n", cost_thrust)
@printf("  Sway cost   = %.3e\n", cost_sway)
