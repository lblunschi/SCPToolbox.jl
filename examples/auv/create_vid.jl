function make_trajectory_video_box_realtime(sol;
    filename::AbstractString="auv_traj_realtime.mp4",
    ...
    cmd = `ffmpeg -y -framerate $(fps) -i $(pattern) -c:v libx264 -preset $(preset) -crf $(crf) -pix_fmt yuv420p $(filename)`
    run(cmd)

    return (filename=filename, tf=float(tf), dt=dt, fps=fps, frames_dir=frames_dir)
end